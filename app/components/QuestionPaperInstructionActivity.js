...
class QuestionPaperInstructionActivity extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            preQuestionId: this.props.navigation.getParam("preQuestionId", ''),
    ...
    async getPaperListAPI() {
        ...
        return fetch(await url(STRING.baseURL) + STRING.getPrePaperQuestions + "prevQuestionId=" + this.state.preQuestionId + "&langId=" + this.state.languageId, {
            method: 'GET',
            headers: {
            ...
            .then((responseJson) => {
                ...
                if (responseJson.length > 0) {
                    this.setState({
                        ...
                        if (this.state.itemList[0].optionA == null || this.state.itemList[0].optionA == '') {
                            this.openPaper();
                        }
                    });
    ...
    openPaper() {
        ...
        if (item.length > 0) {
            if (item[0].optionA == null || item[0].optionA == '') {
                this.props.navigation.replace('solutionPaperActivity', { 'item': item });
            } else {
                this.props.navigation.replace('quizPaperActivity', { 'item': item });
            }
        } else {
    ...
    render() {
        return (
            <View style={{ height: '100%', width: '100%' }}>
                ...
                <Text style={{ marginTop: 50, padding: 12, textAlign: 'left', fontSize: 18, color: COLORS.blue, position: 'absolute' }} >{this.state.paperTitle}</Text>
                    ...
                    <View style={{ height: 70, width: '100%', borderRadius: 14, backgroundColor: COLORS.white, elevation: 5 }}>
                        <View style={{ flexDirection: 'row', padding: 10, marginStart: 10, alignSelf: "flex-start", position: 'absolute' }}>
                            <Image
                                ...
                                source={require('../images/img_que1.png')} />
                        ...
                        <View style={{ flexDirection: 'row', paddingEnd: 20, paddingTop: 10, alignSelf: "flex-end", position: 'absolute' }}>
                            <Image
                                style={{ height: 50, width: 50 }}
                                source={require('../images/img_clock1.png')} />
                            <View style={{ flexDirection: 'column', marginStart: 15 }}>
...
export default QuestionPaperInstructionActivity
