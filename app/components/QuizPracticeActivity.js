...
class QuizPracticeActivity extends Component {
    ...
    async getQuestionDataAPI() {
        ...
        if (this.state.chapterId == '-1') {
            fullURL = await url(STRING.baseURL) + STRING.getSubjectQuiz + "subjectId=" + this.state.subjectId + "&langId=" + this.state.languageId
        } else {
            fullURL = await url(STRING.baseURL) + STRING.getChapterQuiz + "chapterId=" + this.state.chapterId + "&langId=" + this.state.languageId
        }
    ...
    submitQuiz() {
        ...
        db.insertQuizData(code => {
            ...
            this.props.navigation.replace('summeryPracticeActivity', { 'data': this.state.selectedQuestionData, 'questionData': this.state.questionData, 'percentage': (correct * 100) / qData.length, 'correct': correct, 'wrong': qData.length - correct, 'unAnswered': unAnswered })
    ...
    render() {
        return (
            <View style={{ backgroundColor: COLORS.green, height: '100%', width: '100%' }}>
                ...
                <Text style={{ marginTop: 20, padding: 12, alignSelf: 'center', fontSize: 14, color: COLORS.blue, position: 'absolute', fontWeight: 'bold' }} >{getValue(LABEL.practice, this.state.langId)}</Text>
                <View style={GLOBALSTYLE.quizContainer}>
                    {this.state.isLoading
                        ...
                        :
                        <View>
                            <Text style={{ alignSelf: 'center', fontSize: 14, color: COLORS.pink, position: 'absolute', fontWeight: 'bold' }} >{this.state.chapterName}</Text>
                            {this.state.isQuestionAvailable ?
                                <View style={{ flexDirection: 'row', marginTop: 20, alignItems: 'center', alignSelf: 'center' }}>
                                    <Text style={{ fontSize: 12 }}>{getValue(LABEL.timeLeft, this.state.langId)} </Text>
                                    <CountDown
                                        until={this.state.chapterId == '-1' ? 1800 : 1200}
                                        onFinish={() => this.showConfirmDialog(false)}
                                        digitStyle={{ backgroundColor: '#0000000', borderWidth: 0, }}
                                        separatorStyle={{ color: COLORS.black }}
                                        size={12}
                                        timeToShow={['S', 'M']}
                                        timeLabels={{ s: null, m: null }}
                                        showSeparator
                                        running={this.state.shouldRun}
                                    />
                                    <Text style={{ fontSize: 12 }}> {getValue(LABEL.minutes, this.state.langId)}</Text>
                                </View>
                                :
                                <View></View>
                            }
                    ...
                    <Modal
                        ...
                        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
                            <View style={styles.MainAlertView}>
                                ...
                                <Text style={styles.AlertMessage}>{this.state.isTimeUps == false ? getMessage(MESSAGE.submitQuiz, this.state.langId) : getMessage(MESSAGE.timeUp, this.state.langId)}</Text>
...
export default QuizPracticeActivity
