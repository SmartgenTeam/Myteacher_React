...
import Util, { getValue, getMessage } from '../util/Util';
...
class QuizActivity extends Component {
    ...
    getIDDetails() {
        ...
        }, this.state.isPreAssessment ? STRING.PreAssessment : STRING.PostAssessment);
    }
    ...
    async getQuestionDataAPI() {
        ...
        console.log(await url(STRING.baseURL) + STRING.getTopicQuestions + "topicId=" + this.state.topicId + "&langId=" + this.state.languageId);
                ...
                if (responseJson.length > 0) {
                    ...
                    for (let i = 0; i < responseJson.length; i++) {
                        qData.push({
                            ...
                            typeId: this.state.isPreAssessment ? STRING.PreAssessment : STRING.PostAssessment,
    ...
    submitQuiz() {
        ...
        let qData1 = [...this.state.selectedQuestionData], pos = prevPos;

        qData1[pos] = {
            ...qData1[pos],
            totalTimeTaken: (Number(qData1[pos].timeTaken) + (endTime - startTime)),
        ...
        db.insertQuizData(code => {
            ...
            this.props.navigation.replace('summeryActivity', { 'isPreAssessment': this.state.isPreAssessment, 'isFromDetailActivity': this.state.isFromDetailActivity, 'data': this.state.selectedQuestionData, 'questionData': this.state.questionData, 'percentage': (correct * 100) / qData.length, 'correct': correct, 'wrong': qData.length - correct, 'unAnswered': unAnswered })
